...
def sort_events(flight_events_rdd = RDD):
...
def calculate_nav_error(prev_event, current_event):
    t = current_event['time'] - prev_event['time']
    nav_error_x = calculate_for_axis(current_event['nav_x'] - prev_event['nav_x'], current_event['v_x'] - prev_event['v_x'], t)
    nav_error_y = calculate_for_axis(current_event['nav_y'] - prev_event['nav_y'], current_event['v_y'] - prev_event['v_y'], t)
    nav_error_z = calculate_for_axis(current_event['nav_z'] - prev_event['nav_z'], current_event['v_z'] - prev_event['v_z'], t)
...
def calculate_for_axis(axis, v, t):
    return axis - (t * v / 2)


def handle_event(combiner, e):
    if (len(combiner["handled_events"]) > 0):
        error = calculate_nav_error(combiner['last_event'], e)
        ...
        combiner['nav_x_hist'] = add_to_hist(combiner['nav_x_hist'],error['nav_error_x'])
        combiner['nav_y_hist'] = add_to_hist(combiner['nav_y_hist'],error['nav_error_y'])
        combiner['nav_z_hist'] = add_to_hist(combiner['nav_z_hist'],error['nav_error_z'])
        add_sample(combiner,error['nav_total_error'])
...
def add_sample(combiner, sample):
...
def merge_combiners(comb1, comb2):
    ...
    comb1['nav_x_hist'] = merge_hist(comb1['nav_x_hist'],comb2['nav_x_hist'])
    comb1['nav_y_hist'] = merge_hist(comb1['nav_y_hist'],comb2['nav_y_hist'])
    comb1['nav_z_hist'] = merge_hist(comb1['nav_z_hist'],comb2['nav_z_hist'])
    for value in comb2['last_5_samples'].itervalues():
        add_sample(comb1,value)
    return comb1


def merge_states(state1, state2):
    states = (state1 if state1 != None else []) + ([state2] if state2 != None else [])
    return reduce(merge_combiners,states)
...
def add_features(key_with_combiner):
    ...
    print("==== global ====")
    print(globals()['system_start'])

    #print("=============== CURRENT " + str(current_time) )
    #print("=============== SYSTEM " + str(globals()['system_start']))

    #if (current_time - Configuration.system_start_time).seconds >= 10:

    #max_error = max(handled_events, key=get_nav_total_error)
    #for e in handled_events:
    #    e['nav_max_error_10sec'] = max_error
    return handled_events

def get_nav_total_error(e=dict):
    if (e.has_key('nav_total_error')):
        return e['nav_total_error']
    else:
        return None

def handle(flight_events_stream=TransformedDStream, conf=Configuration):
    globals()['system_start'] = conf.system_start_time
